  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-4"
      ...
      <form onSubmit={salvar}
        ...
        {leadNomeFixo && (
          <p className="text-xs" style={{ color: 'var(--text-tertiary)' }}>Lead: <span style={{ color: 'var(--accent)' }}>{leadNomeFixo}</span></p>
        )}

        {precisaSelecionarLead && (
          <div>
            <label className="text-xs block mb-1" style={{ color: 'var(--text-muted)' }}>Lead *</label>
        ...
        <div>
          <label className="text-xs block mb-1" style={{ color: 'var(--text-muted)' }}>Título *</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="text-xs block mb-1" style={{ color: 'var(--text-muted)' }}>Data e hora</label>
        ...
        {mostrarResponsavel && (
          <div>
            <label className="text-xs block mb-1" style={{ color: 'var(--text-muted)' }}>Responsável</label>
        ...
        <div>
          <label className="text-xs block mb-1" style={{ color: 'var(--text-muted)' }}>Descrição</label>
        ...
        {erro && (
          <p className="text-xs px-3 py-2 rounded"
            style={{ background: 'rgba(var(--accent-rgb), 0.1)', color: 'var(--accent-hover)', border: '1px solid rgba(var(--accent-rgb), 0.2)' }}>{erro}</p>
        )}

        <div className="flex gap-2 pt-1">
          <button type="button" onClick={onClose} className="flex-1 text-sm py-2 rounded font-medium"
            style={{ background: 'rgba(var(--ink-rgb), 0.06)', color: 'var(--text)' }}>Cancelar</button>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-4"
      ...
      <form onSubmit={confirmar}
        ...
        style={{ background: 'var(--surface)', border: '1px solid rgba(var(--ink-rgb), 0.08)' }}
        onClick={(e) => e.stopPropagation()}>
        <h2 className="font-bold text-base" style={{ color: 'var(--text)' }}>Concluir tarefa</h2>
        <p className="text-xs" style={{ color: 'var(--text-tertiary)' }}>{tarefa.titulo}</p>

        <div>
          <label className="text-xs block mb-1" style={{ color: 'var(--text-muted)' }}>Resultado</label>
        ...
        <div className="flex gap-2 pt-1">
          ...
          <button type="submit" disabled={salvando} className="flex-1 text-sm py-2 rounded font-medium"
            style={{ background: 'var(--success)', color: '#fff', opacity: salvando ? 0.7 : 1 }}>
