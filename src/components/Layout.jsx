...
function NavGroup({ label }) {
  return (
    <p
      className="px-3 pt-5 pb-1 text-[10px] font-semibold uppercase tracking-widest select-none"
      style={{ color: '#2A2A32' }}
    >
      {label}
    </p>
...
export default function Layout() {
  ...
  function buildNavItems(onItemClick) {
    return (
      <>
        ...
        {['operador', 'gerente'].includes(usuario?.perfil) && (
    ...
    <div className="flex h-screen overflow-hidden" style={{ background: '#08080A' }}>
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-black text-base tracking-widest" style={{ color: '#F4F4F8' }}>NRC</span>
        </div>
        <button
        ...
        >
          <i className={`ti ${menuAberto ? 'ti-x' : 'ti-menu-2'} text-[22px]`} aria-hidden="true" />
        </button>
      </div>
      ...
      {menuAberto && (
        ...
        >
          ...
          >
            <div className="flex items-center gap-2.5 px-4 py-4 mt-14" style={borderBottom}>
              <div
                className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0"
                style={{ background: 'rgba(192,57,43,0.15)' }}
              >
            ...
            <nav className="flex-1 px-2 py-3 overflow-y-auto space-y-0.5">
              {buildNavItems(() => setMenuAberto(false))}
            </nav>
