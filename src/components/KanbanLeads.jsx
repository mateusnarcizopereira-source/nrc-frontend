  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <p className="text-sm font-semibold truncate" style={{ color: 'var(--text)' }}>{lead.nome}</p>
        {lead.temTarefaAtrasada && (
          <i className="ti ti-alarm text-[14px] flex-shrink-0" style={{ color: 'var(--accent-hover)' }} title="Tarefa atrasada" aria-hidden="true" />
        )}
      </div>
      {lead.empreendimento && (
        <p className="text-xs truncate mt-0.5" style={{ color: 'var(--accent)' }}>{lead.empreendimento}</p>
      )}
      <p className="text-[11px] mt-0.5" style={{ color: 'var(--text-muted)' }}>{lead.telefone}</p>
      <p className="text-[10px] mt-1" style={{ color: 'var(--text-muted)' }}>{tempoNoEstagio(lead.statusDesde)}</p>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col flex-shrink-0" style={{ width: '260px' }}>
      ...
      <div ref={setNodeRef}
        ...
        style={{
          background: isOver ? 'rgba(var(--accent-rgb), 0.08)' : 'rgba(var(--ink-rgb), 0.02)',
          border: '1px solid rgba(var(--ink-rgb), 0.05)', borderTop: 'none',
        ...
        {total > limite && (
          <button onClick={() => onCarregarMais(status)} className="w-full text-xs py-1.5 rounded"
            style={{ color: 'var(--text-tertiary)', background: 'rgba(var(--ink-rgb), 0.03)' }}>
            + {total - limite} mais
          </button>
        )}
        {total === 0 && <p className="text-[11px] text-center py-4" style={{ color: 'var(--text-faint)' }}>—</p>}
      </div>
    </div>
  );
}
...
function ZonaDescarte() {
  ...
  return (
    <div ref={setNodeRef} className="flex flex-col items-center justify-center flex-shrink-0 rounded-lg"
      style={{
        width: '120px', border: `1px dashed ${isOver ? 'var(--accent-hover)' : 'rgba(var(--ink-rgb), 0.12)'}`,
        background: isOver ? 'rgba(var(--accent-rgb), 0.10)' : 'transparent', color: isOver ? 'var(--accent-hover)' : 'var(--text-muted)',
      }}>
...
function ModalDescarte({ motivos, onCancel, onConfirm }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.7)' }} onClick={onCancel}>
      <div className="w-full max-w-sm p-5 rounded-xl space-y-3" style={{ background: 'var(--surface)', border: '1px solid rgba(var(--ink-rgb), 0.08)' }} onClick={(e) => e.stopPropagation()}>
        <h3 className="font-bold" style={{ color: 'var(--text)' }}>Marcar como Não Cliente?</h3>
        <p className="text-xs" style={{ color: 'var(--text-tertiary)' }}>Escolha o motivo. Só o corretor responsável pode descartar.</p>
        <div className="space-y-1.5 max-h-52 overflow-y-auto">
          {[...motivos.map((m) => m.texto), 'Outro'].map((m) => (
            <label key={m} className="flex items-center gap-2 p-2 rounded cursor-pointer text-sm"
              style={{ border: '1px solid', borderColor: motivo === m ? 'var(--accent)' : 'rgba(var(--ink-rgb), 0.08)', color: 'var(--text-secondary)' }}>
              <input type="radio" name="mot" checked={motivo === m} onChange={() => setMotivo(m)} className="accent-[var(--accent)]" />
              {m}
            </label>
          ))}
        </div>
        <div className="flex gap-2">
          <button onClick={onCancel} className="flex-1 text-sm py-2 rounded font-medium" style={{ background: 'rgba(var(--ink-rgb), 0.06)', color: 'var(--text)' }}>Cancelar</button>
