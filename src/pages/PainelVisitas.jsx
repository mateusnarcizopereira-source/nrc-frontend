...
export default function PainelVisitas() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-bold" style={{ color: 'var(--text)' }}>Visitas</h1>
      ...
      <div className="grid gap-3">
        {visitas.map((v) => (
          <div
            ...
            style={futuro ? { borderColor: 'rgba(var(--accent-rgb), 0.18)' } : {}}
          >
            <div
            ...
            >
              <p
                ...
                style={{ color: futuro ? 'var(--accent-hover)' : 'var(--text-muted)' }}
              >
                {new Date(v.data + 'T00:00').toLocaleDateString('pt-BR', { day: '2-digit' })}
              </p>
              <p
                className="text-xs"
                style={{ color: futuro ? 'var(--accent)' : 'var(--text-faint)' }}
              >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-2 flex-wrap">
                <div>
                  <p className="font-semibold text-sm" style={{ color: 'var(--text)' }}>{v.empreendimento}</p>
              ...
              {v.comentario && (
                <p className="text-xs mt-1.5 italic" style={{ color: 'var(--text-muted)' }}>{v.comentario}</p>
              )}
            </div>
          </div>
        ))}
